...
var vtalk_dsc = ''
var reply_eva = ''
if (conf.LANG == 'SI') vtalk_dsc = 'EVA VOICE කතාබස් ආරම්භ කරයි.', reply_eva = '*වොයිස් මැසේජයක් ටැග් කරන්න!*'
...
Neutro.tenu({on: 'text', fromMe: wk, donttenuList: true, deleteCommand: false}, (async (message, match) => {
...
Neutro.tenu({on: 'text', fromMe: false, deleteCommand: false}, (async (message, match) => {
...
Neutro.tenu({ pattern: 'vtalk$', desc: vtalk_dsc, fromMe: wk }, (async (message, match) => {
...
var succ_off = ''
if (conf.LANG == 'SI') {
    fulleva_dsc = 'ඔබේ ගිණුම AI CHAT BOT කෙනෙකු කරයි!'
    already_on = 'Eva AI කලින්ම ක්‍රියාත්මක කර ඇත.'
    already_off = 'Eva AI කලින්ම ක්‍රියාවිරහිත කර ඇත.'
    succ_on = 'Eva, සම්පුර්ණයෙන් ක්‍රියාත්මක කර ඇත. රැඳී සිටින්න ✅'
    succ_off = 'Eva,තිබූ ලෙස යලි සකසා ඇත! රැඳී සිටින්න☑️'
}
if (conf.LANG == 'EN') {
...
Asena.tenu({ pattern: 'eva ?(.*)', desc: fulleva_dsc, fromMe: true, usage: '.fulleva on / off' }, (async (message, match) => {
